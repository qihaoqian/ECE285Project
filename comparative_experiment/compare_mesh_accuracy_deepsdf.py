...
def ply_to_checkpoint(ply_path):
    ...
    # 获取父目录（workspace/ngp/bathtub_0025）
    parent_dir = path.parent.parent
    
    # 构造checkpoint路径
    checkpoint_path = parent_dir / "checkpoints" / "deepsdf_ep0030.pth"
...
def sdf_value_loss(checkpoint_path, config_path, mesh_path='data/armadillo.obj', resolution=256):
    ...
    with torch.no_grad():
        for i in range(0, pts.shape[0], B):
            ...
            pred_chunk, _ = model(chunk)            # 只取第一个返回值
            if pred_chunk.dim() == 2:  # 如果是 (B,1) 的形状
                pred_chunk = pred_chunk.squeeze(-1)  # 转换为 (B,)
...
def main(gt_path, pred_path, config_path):
    ...
    with open("comparative_experiment/mesh_accuracy_results_deepsdf.txt", "a") as f:
