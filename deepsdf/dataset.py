...
class SDFDataset(Dataset):
    def __init__(self, path, size=100, 
        ...
        # min_bound = vertices.min(axis=0) # Minimum corner of the bounding box
        # max_bound = vertices.max(axis=0)  # Maximum corner of the bounding box 
        # center = (min_bound + max_bound) / 2
        # bbox_diagonal = np.linalg.norm(max_bound - min_bound)  # Diagonal length of the bounding box
        # scale_factor = 0.4 * np.sqrt(3) / (bbox_diagonal / 2)  # Scale so that the object's diagonal fits within the sphere
    ...
    def __getitem__(self, _):
        ...
        points_surf[self.num_samples_surf // 2:] += 0.01 * np.random.randn(self.num_samples_surf // 2, 3)
